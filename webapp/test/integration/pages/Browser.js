				...
				iShouldSeeTheHashForObjectN: function (iObjIndex) {
					return this.waitFor(this.createAWaitForAnEntitySet({
						...
						success: function (aEntitySet) {
							...
							Opa5.assert.strictEqual(sHash, "PurchaseOrders/" + aEntitySet[iObjIndex].POId, "The Hash is not correct");
						}
					}));
				},
					...
					return this.waitFor({
						success: function () {
							var sObjectId = this.getContext().currentItem.id,
								...
								sHash = oHashChanger.getHash();
							Opa5.assert.strictEqual(sHash, "PurchaseOrders/" + sObjectId, "The Hash is not correct");
						}
					});
				},
